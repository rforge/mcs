...
outputs are post-processed to correct for local and unresolved effects.
Model output statistics (MOS, \citealp{glahn:j_appl_meteorol:72})
develops a statistical relationship between observations of a
meteorological variable to be predicted and the corresponding projected
NWP quantity at some lead time. To refine such a basic MOS,
variable-subset selection is often employed to determine which NWP
outputs should be included in the regression model for a specific location.
