...
\title{\pkg{lmSubsets}: Efficient Computation of Best Subset Linear Regressions in \proglang{R}}
\Plaintitle{lmSubsets: Efficient Computation of Best Subset Linear Regressions in R}
...
The \pkg{lmSubsets} package for exact, best variable-subset regression
implements the algorithms presented by
\cite{Gatu+Kontoghiorghes:2006} and
\cite{Hofmann+Gatu+Kontoghiorghes:2007}.  Computationally intensive core
code is written in \proglang{C++}.  It is available as a package \citep{lmSubsets}
for the \proglang{R} system for statistical computing \citep{R}
from the Comprehensive \proglang{R} Archive
Network at \url{http://CRAN.R-project.org/package=lmSubsets}.

\verb+TODO:+ Somewhere we need to comment on alternative approaches in \proglang{R}:
\begin{itemize}
  \item Other algorithms for linear regression models that do exact search but avoid exhaustive fitting of all models:
    \pkg{leaps} package \citep{leaps} based on \cite{Miller:2002}, and the \pkg{subselect} package
    \citep{subselect} based on \cite{DuarteSilva:2001}.
  \item Exhaustive search for generalized linear models: \pkg{bestglm} \citep{bestglm}.
  \item Approximate solutions via genetic algorithms for generalized linear models and beyond:
     \pkg{glmulti} \citep{Calcagno+DeMazancourt:2010}, \pkg{kofnGA} \citep{Wolters:2015},
     \pkg{mogavs} \citep{Sinha+Malo+Kuosmanen:2015,mogavs}.
  \item Regularized estimation of parametric models with automatic variable selection:
    lasso or elastic net estimation for generalized linear models \citep{Friedman+Hastie+Tibshirani:2010},
    model-based boosting \citep{Buehlmann+Hothorn:2007,Hothorn+Buehlmann+Kneib:2010}.
\end{itemize}

\verb+Just for us:+ Some ideas in the following tutorial (without author or proper
reference) might be useful for inspiration.
...
a criterion of the AIC family \citep{Miller:2002}.  The aforementioned
...
\citep{Gatu+Kontoghiorghes:2006}.
...
a lower RSS \citep{Gatu+Kontoghiorghes:2006}.
...
\citep{Gatu+Kontoghiorghes:2006,Hofmann+Gatu+Kontoghiorghes:2007}.
...
% \proglang{S}3 class \class{lmSubsets} with associated methods

% \fct{lmSubsets} is generic with the default method being the workhorse 
...
% Many methods that work for \class{lm} objects also work for \class{lmSubsets}
...
% \class{lmSubsets} object are assessed.  Currently, the methods to the generic functions include
...
\begin{figure}[t!]
\centering
\includegraphics{lmSubsets-summary}
\caption{BIC and RSS.
\label{fig:summary}}
\end{figure}

\verb+FIXME:+ currently does not work.
