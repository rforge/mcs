...
\begin{table}[t!]
  ...
  \begin{tabular}{llll}
    ...
    \code{tolerance} & BBA tolerance parameters  & \code{double[nvar]}       & \fct{lmSubsets}      \\
...
weather prediction (NWP) models \citep{bauer:nature:15}.
Starting from \cite{glahn:j_appl_meteorol:72} the outputs from these
physically-based large-scale (typically global) NWP models are
statistically post-processed to correct small-scale biases and obtain
predictions for specific locations. Such model output statistics (MOS)
relate past meteorological observations to the corresponding NWP
quantity but can also include further quantities as additional regressors.
Variable-subset selection is often employed to determine which
NWP quantities should be included in the MOS regression model
for a specific location.

In the following, we use such \pkg{lmSubsets} to build a MOS regression
model predicting temperature at Innsbruck Airport, Austria, based on a wide
range of NWP quantities from the Global Ensemble Forecast
...
data frame \code{IbkTemperature} contains 1824 daily cases
for the observed response (in degree Celsius), 36 NWP outputs (forecasted),
and five deterministic time trend/season patterns. The NWP variables include
several temperature quantities (in degree Kelvin, e.g., 2-meter, minimum,
maximum, soil) as well as several quantities capturing precipitation, wind,
and fluxes among others. See \code{?IbkTemperature} for more details.

First, the dataset is loaded and for simplicity a couple of missing values
are omitted.
<<data>>=
...
(\code{temp}) is constructed, which will serve here as a reference model.
It consists of the 2-meter temperature NWP forecast (\code{t2m}), a linear
trend component (\code{time}), as well as harmonic seasonal components at
annual (\code{sin}, \code{cos}) and bi-annual (\code{sin2}, \code{cos2})
frequencies.
<<mos0>>=
...
<<mos2-all, echo=FALSE>>=
MOS2.all <- lmSubsets(temp ~ ., data = IbkTemperature)
MOS2 <- refit(lmSelect(MOS2.all, penalty = "BIC"))
@
...
Next, variable-subset models with the lowest RSS for all submodel
sizes are computed and the \class{lm} object corresponding to the
submodel with the lowest BIC score is extracted. Two ``flavors''
of variable selection are considered: (1) The variables from the
based \code{MOS0} (based on the direct model output and climatology)
are kept in the model and all-subsets regression is employed to select
the relevant regressors from the remaining 35 NWP variables.
<<>>=
<<mos1-all>>
@
(2) All variables are considered in the subset selection without
restriction (i.e., searching models with (2--42 regressors, including
the intercept).
<<>>=
<<mos2-all>>
@
The best-BIC models \code{MOS1} and \code{MOS2} both have 13 regressors,
which are listed in Table~\ref{tab:coefs}. Up to eight additional
NWP outputs are selected, six of which are included in both selections:
quantities relating to temperature (\code{tmax2m}, \code{st}, \code{t2pvu}),
pressure (\code{mslp}, \code{p2pvu}), hydrology (\code{vsmc}, \code{wr}), and
heat flux (\code{sshnf}). The deterministic trend and most of the harmonic
seasonal components are retained also in \code{MOS2}. However, and most
remarkably, \code{MOS1} does not include the direct 2-meter temperature
output from the NWP (\code{t2m}). The summary statistics reveal that
both \code{MOS1} and \code{MOS2} significantly improves over the simple
reference model \code{MOS0} with \code{MOS2} begin very slightly better
than \code{MOS1}. 

The RSS and BIC of the variable-subset models for all sizes 2--42
in \code{MOS2.all} are illustrated in Figure~\ref{fig:mos2:plot},
showing again that the BIC minimum is attained at 13 regressors.

\begin{figure}[t!]
\centering
\setkeys{Gin}{width=0.6\textwidth}
<<fig-mos2-plot, fig=TRUE, echo=FALSE>>=
plot(MOS2.all)
@
...
Furthermore, the 20 overall best MOS subset models are computed in
\code{MOS2.best} and illustrated by a selection heatmap in
Figure~\ref{fig:mos2:image}.
<<mos2-best>>=
MOS2.best <- lmSelect(temp ~ ., data = IbkTemperature, penalty = "BIC",
  nbest = 20)
@
A selected variable is indicated by a dark cell with the number of
selected variables per submodel ranging from 11 to 14. Note that
\code{t2m} has not been picked up by any of the submodels but instead
\code{tmax2m}, \code{st}, and \code{t2pvu} are included in all of them.

\begin{figure}[t!]
\centering
\setkeys{Gin}{width=\textwidth}
<<fig-mos2-image, fig=TRUE, echo=FALSE, height=6, width=12>>=
image(MOS2.best, col = c("gray50", "gray90"), lab = c("bold(lab)", "lab"),
  hilite = 1, hilite_col = "red")
@
\caption{Subset selection for \code{MOS2.best}.}
\label{fig:mos2:image}
\end{figure}
...
forecasting skill. In a full meteorological application this should,
of course, be further tested using cross-validation or other
out-of-sample assessments. Similarly, it is worth remarking that
recently there has been increasing interest in MOS models beyond
least-squares linear regression, e.g., for incorporating heteroscedasticity,
censoring, or truncation. As all-subsets regression is much more
burdensome in such models, other selection techniques such as boosting can
be considered \citep[e.g., as available in the \pkg{crch} package,][]{messner:r_j:16,messner:mon_wea_rev:17}.
A comparison with such approaches, however, is beyond the scope of this
paper.
