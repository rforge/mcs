...
($k>0$).  The standard AIC and BIC criteria are obtained for $k=2$ and
$k=\log M$, respectively~\citep{miller:02}.  It follows
...
thus lowering the compuational cost.  On the other hand, by explicitly
solving the more costly all-subsets regression
problem~\eqref{eq:all_subsets} once and for all, the list of all $N$
submodels is made readily available for further exploration:
evaluating multiple criterion functions (e.g., AIC and BIC), or
performing a more elaborate statistical inference, then comes at a
negligible cost.  Thus, it can be advisable to adopt a two-stage
approach in the case of a broader and more thorough statistical
investigation.

Brute-force (or exhaustive) search procedures that enumerate all
possible subsets to solve~\eqref{eq:best_subset}
and~\eqref{eq:all_subsets} become intractable even for a modest number
of variables.  Exact algorithms must employ techniques to reduce the
size of the search space -- i.e., the number of enumerated subsets --
in order to tackle larger problems.  Heuristic algorithms trade
optimality for speed: they are designed for solving a problem more
quickly, but make no guarantees on the quality of the solution
produced; genetic algorithms and simulated annealing count among the
well-known heuristic strategies.  Approximation algorithms, on the
other hand, return a solution that provably lies within well specified
...
based on \cite{miller:02}.  Exhaustive algorithms have been considered
within the context of generalized linear models by the package
\pkg{bestglm}~\citep{bestglm}.  The package \pkg{subselect} proposes
simulated annealing algorithms based on
\cite{duarte_silva:j_multivariate_anal:01}.  Furthermore, genetic
algorithms for generalized linear models and beyond have been
implemented by the packages
\pkg{glmulti}~\citep{calcagno:j_stat_softw:10} and
\pkg{kofnGA}~\citep{wolters:j_stat_softw:15}.  Non-exact algorithms
for regularized estimation of parametric models with automatic
variable selection performed by lasso or elastic net estimation for
generalized linear models can be found
in~\cite{friedman:j_stat_softw:10}.

Here, the \pkg{lmSubsets} package for exact variable-subset regression
is presented.  It offers methods for solving both the {\em
...
proposes approximation methods that compute non-exact solutions very
quickly while giving guarantees on the quality of the results.  The
core of the package is written in \proglang{C++}.  The package is
...
underlying algorithms.  Section~\ref{sec:R} presents the functions,
methods and objects part of the package's {\R} interface.  A usage
example is given in Section~\ref{sec:usecase:1}, while benchmark
results are illustrated in Section~\ref{sec:benchmark}.
...
corresponds to Akaike's information criterion
