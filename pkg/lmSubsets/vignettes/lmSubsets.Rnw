...
available today.  To obtain forecasts for specific locations, the NWP
outputs are post-processed to correct for local effects and for
certain model biases~\citep{barry:03}.  Model output statistics (MOS)
develops a statistical relationship between the variable to be
estimated (observed quantity) and selected variables from the
numerical model at some projection time (forecasted
quantities)~\citep{glahn:j_appl_meteorol:72}.  MOS employs
variable-subset selection to determine which NWP variables should be
...
First, the dataset is loaded and missing values are omitted for
simplicity.
<<data>>=
