...
quality and execution time.  Note that ``heuristic BBA'' is a
...
\end{Code}
%
using standard {\R} formula notation, where the dot (\code{.}) stands
for ``all variables not mentioned in the left-hand side of the
formula''.  By default, an intercept term is included in the model;
that is, the call in the previous example is equivalent to
%
...
model parameters are penalized, with higher values favoring
...
homonomic parameters of the \code{lm} function.  The \code{include}
...
variable -- or are automatically expanded to such if given in the form
...
discarded.  Well-defined regions of the regression tree can be ignored
by the selection algorithm, and the search space is thus reduced.
...
and \code{best}, which specify the number of regressors in, and the
