...
namespace detail {
  ...
  template<typename TReal>
  void
  Qrz<TReal>::permute2(int n,
                       const TReal* const rz, const int ldrz, const int j,
                             TReal* const sz, const int ldsz)
  {
    std::copy_n(rz + j * ldrz, j + 1, sz);
    std::fill_n(sz + j + 1, n - j, 0);

    for (int i = 0; i < j; ++i)
      {
        std::copy_n(rz + i * ldrz, i + 1, sz + (i + 1) * ldsz);
        std::fill_n(sz + (i + 1) * ldsz + i + 1, n - i, 0);
      }

    for (int i = j + 1; i < n; ++i)
      {
        std::copy_n(rz + i * ldrz, i + 1, sz + i * ldsz);
        std::fill_n(sz + i * ldsz + i + 1, n - i, 0);
      }
